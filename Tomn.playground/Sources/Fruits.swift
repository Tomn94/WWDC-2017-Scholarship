...
public class FruitsGameScene: SKScene, SKPhysicsContactDelegate {
    
    public  var fruits = ["🍏", "🍎", "🍐", "🍊", "🍋", "🍌", "🍉", "🍇", "🍓", "🍈", "🍒", "🍑", "🍍", "🍆"]
    public  var specialFruits = ["🌮", "🌯", "🍗", "🍕", "🍔", "🍟"]
    public  var loseLife = ["💣"]
    
    public  var spawnTimeProbability: UInt32 = 50
    public  var bonusProbability: UInt32 = 2
    public  var specialProbability: UInt32 = 10
    public  var bombInitialProbability: UInt32 = 10
    
    public  var startLives = 3
    ...
    override public func didMove(to view: SKView) {
        ...
        lives = startLives > livesMax ? livesMax : startLives
        if lives < 0 {
            lives = 0
        }
        updateLives(0)
    ...
    override public func update(_ currentTime: TimeInterval) {
        ...
        let timeLimit = (Double(arc4random_uniform(spawnTimeProbability)) + 0.1) * (0.2 + difficultéMax - difficulté)
        if currentTime - timeLastFruit > timeLimit {
            timeLastFruit = currentTime
            
            let randAppear = arc4random_uniform(200)
            if randAppear < bonusProbability {
                showBonus()
            } else if randAppear < specialProbability {
                showSpecialFruit()
            } else if randAppear < bombInitialProbability + UInt32(ceil(difficulté) * 2) {
                showBomb()
            } else {
                showFruit()
            }
        ...
        if let bodyA = contact.bodyA.node,
           let bodyB = contact.bodyB.node {
            if bodyA.parent != nil && bodyB.parent != nil {
                ...
                if nodeNames.contains(where: {$0 == "blade"}) {
...
class Fruit: SKLabelNode {
    convenience init(scene: FruitsGameScene) {
        self.init()
        
        name = "fruit"
        text = scene.fruits[Int(arc4random_uniform(UInt32(scene.fruits.count)))]
        fontSize = 72
...
class SpecialFruit: Fruit {
    convenience init(theScene: FruitsGameScene) {
        self.init(scene: theScene)
        
        userData = ["specialFruit": true]
        text = theScene.specialFruits[Int(arc4random_uniform(UInt32(theScene.specialFruits.count)))]
    }
}
...
class Bomb: SKLabelNode {
    convenience init(scene: FruitsGameScene) {
        ...
        name = "bomb"
        text = scene.loseLife[Int(arc4random_uniform(UInt32(scene.loseLife.count)))]
        fontSize = 72
